...
parser.add_argument("--SC", dest="SC_mag", help="Deactivate SC muon shield. Configuration: 1 SC magnet (3*B_warm) + 3 warm magnets with inverted fields", action='store_true')
...
if options.charm == 0: ship_geo = ConfigRegistry.loadpy("$FAIRSHIP/geometry/geometry_config.py", Yheight = options.dy, tankDesign = options.dv, \
                                                ...
                                                SC_mag=options.SC_mag, scName=options.scName, params=shield_db[options.scName], decayVolumeMed=options.helium)
