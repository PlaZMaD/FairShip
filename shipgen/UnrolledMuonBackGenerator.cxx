...
#include "UnrolledMuonBackGenerator.h"
...
Bool_t UnrolledMuonBackGenerator::Init(const char* fileName, const int firstEvent, std::string json_input, const Bool_t fl = false ) {
  ...
  std::stringstream jsonEncodedData(json_input);
  boost::property_tree::read_json(jsonEncodedData, input_config);
  ...
  if (fTree){
   fNevents = eventList.size();//fTree->GetEntries();
...
// -------------------------------------------------------------------------
Bool_t UnrolledMuonBackGenerator::checkDiMuon(Int_t muIndex){
   Bool_t check = false;
...
// -----   Passing the event   ---------------------------------------------
Bool_t UnrolledMuonBackGenerator::ReadEvent(FairPrimaryGenerator* cpg)
{
...
// -------------------------------------------------------------------------
Int_t UnrolledMuonBackGenerator::GetNevents()
{
 return fNevents;
}
void UnrolledMuonBackGenerator::CloseFile()
{
 fInputFile->Close();
 fInputFile->Delete();
 delete fInputFile;
...
ClassImp(UnrolledMuonBackGenerator)
