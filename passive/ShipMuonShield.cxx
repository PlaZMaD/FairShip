...
void ShipMuonShield::CreateMagnet(TString magnetName,TGeoMedium* medium,TGeoVolume *tShield,TGeoUniformMagField *fields[4],FieldDirection fieldDirection,
          ...
          Bool_t stepGeo)
  {
    ...
    if (NotMagnet) {
       ...
       std::cout << "GAPS LIM:" << CalculateGapWidth(2 * dY, 2 * dX) << "," << CalculateGapWidth(2 * dY2, 2 * dX2) << std::endl;
       gap = std::ceil(std::max(CalculateGapWidth(2 * dY, 2 * dX), gap));
       gap2 = std::ceil(std::max(CalculateGapWidth(2 * dY2, 2 * dX2), gap2));
       coil_gap = gap;
       coil_gap2 = gap2;
    } else {
